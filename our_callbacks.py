...
'step_decay', 'step_decay_two', 'step_decay_three',
...
class LearningRateScheduler(Callback):
    ...
    def on_batch_begin(self, batch, log):
        ...
        if self.method == "linear":
            lr = self.start_lr + (self.end_lr-self.start_lr)*iteration/(self.iterations*self.args.epochs)
        elif self.method == "abs_sin":
            lr = self.start_lr*math.fabs( (iteration/(self.iterations*self.args.epochs) -1)*math.sin(2/((iteration/(self.iterations*self.args.epochs) -1)+1e-6)) )
        elif self.method == 'cos_tanh':
            cos = (self.start_lr+self.end_lr)/2.+(self.start_lr-self.end_lr)/2.*math.cos(math.pi/2.*(iteration/(self.iterations*self.args.epochs/2.))) 
            tanh = (self.start_lr+self.end_lr)/2. - (self.start_lr-self.end_lr)/2. * math.tanh(8.*iteration/(self.iterations*self.args.epochs) - 4.)
            lr = (self.args.cosine_weight*cos+(1-self.args.cosine_weight)*tanh)
        elif self.method == "cos_iteration":
        ...
        elif self.method == 'tanh_restart_tanhdecay':
            #  tanh as max and 0.1*tanh as min, multiply tanh
            max_lr = (self.start_lr+self.end_lr)/2. - (self.start_lr-self.end_lr)/2. * math.tanh(8.*iteration/(self.iterations*self.args.epochs) - 4.)
            min_lr = 0.1*max_lr
            dt = 1./(self.T_e*self.iterations)
            self.tt = self.tt+dt
            lr = (max_lr+min_lr)/2. - (max_lr-min_lr)/2. * math.tanh(8.*self.tt - 4.)
        elif self.method == 'cos_restart_tanhdecay':
    ...
    def on_epoch_begin(self, epoch, log):
        ...
        elif self.method == "step_decay_two":
            self.learning_rate_scheduler[1]=[0, 162, 300]
            for i in range(len(self.learning_rate_scheduler[1])):
                if self.learning_rate_scheduler[1][i] <= epoch < self.learning_rate_scheduler[1][i+1]:
                    lr = self.learning_rate_scheduler[0][i]
        elif self.method == "step_decay_three":
            self.learning_rate_scheduler[1]=[0, 122, 300]
            for i in range(len(self.learning_rate_scheduler[1])):
                if self.learning_rate_scheduler[1][i] <= epoch < self.learning_rate_scheduler[1][i+1]:
                    lr = self.learning_rate_scheduler[0][i]
